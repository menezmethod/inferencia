...
OpenAI-compatible REST API for chat completions, embeddings, and model management. Deploy your own instance (e.g. with Coolify). Default chat model fallback (when `model` is omitted): **qwen3:6b-bf16**.
...
```json
{
  ...
  "data": [
    ...
    { "id": "qwen3:6b-bf16", "object": "model", "created": 0, "owned_by": "ollama" },
...
Use `qwen3:6b-bf16` (or another Ollama model tag available in your local runtime).
