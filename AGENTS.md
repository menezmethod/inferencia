...
- **Lint**: `make lint` (golangci-lint); also run in CI (Build & test job). CI runs Build & test, Integration (Ginkgo + Newman + Docker/Compose smoke), Sensitive data (blocklist + gitleaks). `make check-compose` validates Coolify docker-compose policy locally.
