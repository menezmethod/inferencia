...
gh api repos/:owner/:repo/branches/main/protection -X PUT \
  ...
  --silent 2>/dev/null || {
  echo "Branch protection API failed (e.g. status checks not yet run). Set manually in GitHub:"
  echo "  Settings → Branches → Add rule for 'main' → Require status checks: Build & test, Lint, Integration (Docker smoke)"
}
